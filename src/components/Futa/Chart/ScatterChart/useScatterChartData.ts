...
const useScatterChartData = () => {
    ...
    const data = useMemo(() => {
        const localData = globalAuctionList.data
            ?.map((element) => {
                const filledClearingPriceInWeiBigInt =
                    element.filledClearingPriceInNativeTokenWei
                        ...
                        : undefined;
                const filledMarketCapInWeiBigInt =
                    filledClearingPriceInWeiBigInt
                        ? filledClearingPriceInWeiBigInt *
                ...
                const auctionEndTime =
                    element.createdAt + element.auctionLength;
                const timeRemainingInSec =
                    auctionEndTime - currentTimeInSeconds;

                return {
                    name: element.ticker,
                    price: filledMarketCapUsdValue
