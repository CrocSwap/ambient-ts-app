...
export const fetchPoolLiquidity = async (
  ...
  return fetch(
    ...
    .then(async (json) => {
      ...
      return await expandLiquidityData(bumps, base, quote, poolIdx, chainId, crocEnv, cachedFetchTokenPrice);
    });
};
...
function bumpsToRanges(
  ...
  let ranges = bumps.map((b) => {
    ...
    const lowerPriceDisp = toDisplayPrice(lowerPrice, baseDecimals, quoteDecimals);
    const upperPriceDisp = toDisplayPrice(upperPrice, baseDecimals, quoteDecimals);
    ...
    const deltaSqrtPrice =
      retVal.lowerBound >= tick
        ? Math.sqrt(retVal.upperBoundPrice) - Math.sqrt(retVal.lowerBoundPrice)
        : Math.sqrt(retVal.lowerBoundPrice) - Math.sqrt(retVal.upperBoundPrice);
    const deltaSqrtInvPrice =
      retVal.upperBound <= tick
        ? Math.sqrt(retVal.upperBoundInvPrice) - Math.sqrt(retVal.lowerBoundInvPrice)
        : Math.sqrt(retVal.lowerBoundInvPrice) - Math.sqrt(retVal.upperBoundInvPrice);
    ...
    const deltaQuoteDecimal = retVal.deltaQuote / Math.pow(10, quoteDecimals);
    const deltaBaseUSD = deltaBaseDecimal * basePrice;
    const deltaQuoteUSD = deltaQuoteDecimal * quotePrice;

    retVal.deltaAverageUSD = (Math.abs(deltaBaseUSD) + Math.abs(deltaQuoteUSD)) / 2;
  ...
  ranges.forEach((r) => {
    ...
    const cumDeltaQuoteDecimal = r.cumDeltaQuote / Math.pow(10, quoteDecimals);
    const cumDeltaBaseUSD = cumDeltaBaseDecimal * basePrice;
    const cumDeltaQuoteUSD = cumDeltaQuoteDecimal * quotePrice;
    r.cumAverageUSD = (Math.abs(cumDeltaBaseUSD) + Math.abs(cumDeltaQuoteUSD)) / 2;
  });
