...
export async function fetchPoolList(crocEnv: CrocEnv, graphCacheUrl: string): Promise<GCServerPoolIF[]> {
  ...
  return fetch(ENDPOINT)
    ...
    .then((json) => {
      ...
      const pools: Promise<GCServerPoolIF[]> = Promise.allSettled(payload).then((results) => {
        function getFulfilledValues<T>(promises: PromiseSettledResult<T>[]): T[] {
          ...
          IS_LOCAL_ENV &&
            rejectedPromises.forEach((reject: PromiseRejectedResult) => {
              console.warn('failed pool metadata query, see file fetchPoolList.ts to troubleshoot', reject);
            });
