...
const fetchPoolStats = async (
  ...
  return fetch(
    ...
    .then((json) => {
      ...
      return expandPoolStats(payload, base, quote, poolIdx, chainId, crocEnv, cachedFetchTokenPrice);
    });
};
...
async function expandPoolStats(
  ...
  return decoratePoolStats(payload, await pool.baseDecimals, await pool.quoteDecimals, basePrice, quotePrice);
}
...
function decoratePoolStats(
  ...
  stats.quoteVolumeDecimal = payload.quoteVolume / Math.pow(10, quoteDecimals);
...
const get24hChange = async (
  ...
  if (ydayPrice && nowPrice && ydayPrice > 0 && nowPrice > 0) {
    return denomInBase ? ydayPrice / nowPrice - 1.0 : nowPrice / ydayPrice - 1.0;
  } else {
    return 0.0;
  }
};
...
export async function getChainStats(
  ...
  return fetch(
    ...
    .then((json) => {
      ...
      return expandChainStats(payload, chainId, crocEnv, cachedFetchTokenPrice);
    })
    .catch((e) => {
      console.warn(e);
      return undefined;
...
async function expandChainStats(
  ...
  const subAggs = await Promise.all(
    tokenStats.map((t) => expandTokenStats(t, chainId, crocEnv, cachedFetchTokenPrice))
  );
...
async function expandTokenStats(
  ...
  const usdPrice = cachedFetchTokenPrice(stats.tokenAddr, chainId).then((p) => p?.usdPrice || 0.0);
