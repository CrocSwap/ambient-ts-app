...
export const calculateSecondaryDepositQty = (
  ...
  const isPrimaryTokenBase = (isTokenAPrimary && isTokenABase) || (!isTokenAPrimary && !isTokenABase);
  ...
  if (secondaryQuantity) {
    if (secondaryQuantity === Infinity || secondaryQuantity === -Infinity || isNaN(secondaryQuantity)) {
      return 0;
    } else {
