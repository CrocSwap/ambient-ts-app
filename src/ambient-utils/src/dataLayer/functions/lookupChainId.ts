...
export function lookupChainId(rawInput: string | number, returnAs: 'string'): string | undefined;

// fn signature to return type 'number'
export function lookupChainId(rawInput: string | number, returnAs: 'number'): number | undefined;
...
export function lookupChainId(
  ...
  const inputAsString: string = typeof rawInput === 'string' ? rawInput : '0x' + rawInput.toString(16);
