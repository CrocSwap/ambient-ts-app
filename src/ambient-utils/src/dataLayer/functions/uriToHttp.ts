...
export function uriToHttp(uri: string, retry: 'retry'): string[];

export function uriToHttp(uri: string, retry?: string): string | string[] {
  ...
  try {
    ...
    const ipfsBases: string[] = ['https://cloudflare-ipfs.com/ipfs/', 'https://ipfs.io/ipfs/'];

    // base URLs for IPNS queries
    const ipnsBases: string[] = ['https://cloudflare-ipfs.com/ipns/', 'https://ipfs.io/ipns/'];
    ...
    switch (protocol) {
      ...
      // handle ipfs URIs
      case 'ipfs':
        ipfsBases.forEach((base: string) => outputURLs.push(base + makeHash(uri)));
        break;
      // handle ipns URIs
      case 'ipns':
        ipnsBases.forEach((base: string) => outputURLs.push(base + makeHash(uri)));
        break;
      default:
        outputURLs.push('');
        break;
