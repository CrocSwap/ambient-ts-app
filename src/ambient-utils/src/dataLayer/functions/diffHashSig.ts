...
export function diffHashSigScaleData(scaleData?: ScaleDataIF, axis: string | undefined = undefined) {
  if (!scaleData) {
    return 'null';
  }
...
export function diffHashSigLimits(txs?: LimitOrderIF[]) {
  ...
  return diffHashSig(
    txs.map((x) => ({ limitOrderId: x.limitOrderId, latestUpdateTime: x.latestUpdateTime }))
  );
}
...
export function diffHashSigPostions(txs?: PositionIF[]) {
  ...
  return diffHashSig(txs.map((x) => ({ positionId: x.positionId, latestUpdateTime: x.latestUpdateTime })));
}
