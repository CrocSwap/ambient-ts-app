...
export const getTransactionData = async (
  ...
  const quoteTokenAddress = tx.quote.length === 40 ? '0x' + tx.quote : tx.quote;
  ...
  newTx.ensResolution = skipENSFetch ? '' : (await cachedEnsResolve(tx.user)) ?? '';
  ...
  newTx.baseFlowDecimalCorrected = tx.baseFlow / Math.pow(10, baseTokenDecimals);
  newTx.quoteFlowDecimalCorrected = tx.quoteFlow / Math.pow(10, quoteTokenDecimals);
  ...
  newTx.limitPrice = newTx.isBuy ? tickToPrice(newTx.bidTick) : tickToPrice(newTx.askTick);
  newTx.limitPriceDecimalCorrected = toDisplayPrice(newTx.limitPrice, baseTokenDecimals, quoteTokenDecimals);
