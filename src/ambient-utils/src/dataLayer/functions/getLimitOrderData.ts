...
export const getLimitOrderData = async (
  ...
  newOrder.ensResolution = skipENSFetch ? '' : (await cachedEnsResolve(order.user)) ?? '';
  ...
  const baseTokenDecimals = (await baseMetadata)?.decimals ?? DEFAULT_DECIMALS;
  const quoteTokenDecimals = (await quoteMetadata)?.decimals ?? DEFAULT_DECIMALS;
  ...
  newOrder.originalPositionLiqBase = bigNumToFloat(
    baseTokenForConcLiq(
      !order.isBid ? tickToPrice(order.bidTick - 1) : tickToPrice(order.askTick + 1),
  ...
  newOrder.positionLiqBaseDecimalCorrected = newOrder.positionLiqBase / Math.pow(10, baseTokenDecimals);
  newOrder.positionLiqQuoteDecimalCorrected = newOrder.positionLiqQuote / Math.pow(10, quoteTokenDecimals);
  ...
  newOrder.claimableLiqBaseDecimalCorrected = newOrder.claimableLiqBase / Math.pow(10, baseTokenDecimals);
  newOrder.claimableLiqQuoteDecimalCorrected = newOrder.claimableLiqQuote / Math.pow(10, quoteTokenDecimals);

  const basePrice = await basePricePromise;
  const quotePrice = await quotePricePromise;
  const poolPrice = toDisplayPrice(await poolPriceNonDisplay, baseTokenDecimals, quoteTokenDecimals);

  newOrder.limitPrice = order.isBid ? tickToPrice(order.bidTick) : tickToPrice(order.askTick);
  ...
  newOrder.invLimitPriceDecimalCorrected = 1 / newOrder.limitPriceDecimalCorrected;

  const totalBaseLiq = newOrder.positionLiqBaseDecimalCorrected + newOrder.claimableLiqBaseDecimalCorrected;
  const totalQuoteLiq =
    newOrder.positionLiqQuoteDecimalCorrected + newOrder.claimableLiqQuoteDecimalCorrected;

  if (quotePrice && basePrice) {
    newOrder.totalValueUSD = quotePrice.usdPrice * totalQuoteLiq + basePrice.usdPrice * totalBaseLiq;
  } else if (basePrice) {
    const quotePrice = basePrice.usdPrice * poolPrice;
    newOrder.totalValueUSD = quotePrice * totalQuoteLiq + basePrice.usdPrice * totalBaseLiq;
  } else if (quotePrice) {
    const basePrice = quotePrice.usdPrice / poolPrice;
    newOrder.totalValueUSD = basePrice * totalBaseLiq + quotePrice.usdPrice * totalQuoteLiq;
  } else {
