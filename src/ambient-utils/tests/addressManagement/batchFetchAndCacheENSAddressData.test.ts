import { batchFetchAndCacheENSAddressData } from '../../dataLayer/addressManagement/batchFetchAndCacheENSAddressData';
...
describe('batchFetchAndCacheENSAddressData', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        (getAddress as jest.Mock).mockImplementation((addr) =>
            addr.toLowerCase(),
        );
    });
    ...
    it('should not fetch when there are no uncached addresses', async () => {
        ...
        const result = await batchFetchAndCacheENSAddressData(
            data,
            addressCache,
            nullAddressCache,
        );
    ...
    it('should not fetch when all addresses are cached', async () => {
        const data: Partial<TradeTableDataRow>[] = [
            { user: '0xSomeAddress1' },
            { user: '0xSomeAddress2' },
        ];
        const addressCache = new Map([
    ...
    it('should fetch when addresses are not in cache', async () => {
        ...
        (fetchEnsAddresses as jest.Mock).mockResolvedValueOnce(
            new Map([['0xsomeaddress3', 'name3.eth']]),
        );
        ...
        expect(result.updatedAddressesCache.size).toBe(2);
        expect(result.updatedAddressesCache.get('0xsomeaddress3')).toBe(
            'name3.eth',
        );
        expect(result.updatedAddressesCache.get('0xsomeaddress4')).toBe(
            'name4.eth',
        );
    });
    ...
    it('should handle addresses that returned null correctly', async () => {
        ...
        expect(
            result.updatedNullAddressesCache.has('0xsomeaddress5'),
        ).toBeTruthy();
    });
});
