...
export const querySpotPrice = async (
    ...
    if (!crocEnv || (await crocEnv.context).chain.chainId !== _chainId) return;
...
export function memoizeQuerySpotTick(): PoolQueryFn {
    return memoizeCrocEnvFn(querySpotTick) as PoolQueryFn;
}
