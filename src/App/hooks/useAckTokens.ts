...
export const useAckTokens = (): ackTokensMethodsIF => {
    ...
    function acknowledgeToken(newTkn: TokenIF): void {
        const ackTokensWithNewRemoved: TokenIF[] = ackTokens.filter(
            (ackToken: TokenIF) =>
                ackToken.address.toLowerCase() !==
                    newTkn.address.toLowerCase() ||
                ackToken.chainId === newTkn.chainId,
        );
        const updatedAckTokensArray: TokenIF[] = [
            newTkn,
            ...ackTokensWithNewRemoved,
        ];
        setAckTokens(updatedAckTokensArray);
        localStorage.setItem(
            localStorageKey,
            JSON.stringify(updatedAckTokensArray),
        );
    }

    // fn to check if a token has been acknowledged
    function checkToken(addr: string, chn: number | string): boolean {
        const chnAsNumber: number =
            typeof chn === 'string' ? parseInt(chn) : chn;
        return ackTokens.some(
            (ackToken: TokenIF) =>
