...
export const useSidebarSearch = (
    ...
    useEffect(() => {
        ...
        const searchBySymbol = (symb: string): TempPoolIF[] =>
            poolList.filter((pool: TempPoolIF) =>
                symb.length === 2
                ...
                tokens.verify(pool.base, pool.chainId) &&
                tokens.verify(pool.quote, pool.chainId)
            );
        // fn to return list of verified pools with no search filtering
        const noSearch = (): TempPoolIF[] => poolList.filter((pool: TempPoolIF) =>
                tokens.verify(pool.base, pool.chainId) &&
                tokens.verify(pool.quote, pool.chainId)
        );
