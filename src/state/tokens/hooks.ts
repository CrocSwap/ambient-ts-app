...
export function useAddTokenKeys(): (addresses: string[]) => void {
    ...
    return useCallback(
        (tokenAddresses: string[]) =>
            dispatch(addTokenKeys({ tokenAddresses, networkId: activeNetwork.id })),
        [activeNetwork.id, dispatch],
    );
}
...
export function usePoolsForToken(address: string): string[] | undefined {
    ...
    const token = useSelector(
        (state: AppState) => state.tokens.byAddress[activeNetwork.id]?.[address],
    );
