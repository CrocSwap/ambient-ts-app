...
//----------------------------------------------------------click ----------------------------------------------
    public async click_limit_btn() {
        await this.page.locator(
            "[id='link_to_limit_module']"
        ).click();
    }

    public async click_pool_btn() {
        await this.page.locator(
            "[id='link_to_pool_module']"
        ).click();
    }
    ...
    public async click_recent_pools() {
        await this.page.locator(
            "[id='sidebar_header_recent_pools']"
        ).click();
    }

    public async click_toppools_token() {
        await this.page.locator(
            "[id='sidebar_header_top_pools']"
        ).click();
    }
...
//-------------------------------------------------assert --------------------------------------------------------
    ...
    public async assert_limit_order() {
        await expect(this.page.locator(
            "[id='sidebar_header_limit_orders']"
        )).toHaveProperty("open", "");
    }

    public async assert_range_position() {
        await expect(this.page.locator(
            "[id='sidebar_header_liquidity_positions']"
        )).toHaveProperty("open", "");
    }
    ...
    public async assert_fav_pools() {
        await expect(this.page.locator(
            "[id='sidebar_header_favorite_pools']"
        )).toHaveProperty("open", "");
    }
